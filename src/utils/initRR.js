...
module.exports = async (bot) => {
  for (var rolechan in roleMap) {
    if (roleMap[rolechan].__info.msg) {
      ...
      var msg = await bot.channels.get(roleMap[rolechan].__info.chan).fetchMessage(roleMap[rolechan].__info.msg)
      for (var role in roleMap[rolechan]) {
        if (role!="__info") {
          if (roleMap[rolechan][role].reactname) {
            ...
            msg.react(msg.guild.emojis.get(roleMap[rolechan][role].react));
          } else {
